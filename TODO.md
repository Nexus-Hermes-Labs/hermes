...
| 5   | Grafana dashboard provisioning                     | `[ ]` | P1       | Auto-provision dashboards under `hermes-be/infra/grafana/provisioning/dashboards/`: per-service error rate (logs `level=ERROR`), log volume by service, recent errors panel, p50/p95 HTTP latency from Prometheus. No hand-built dashboards in Explore. **Includes prerequisite metrics-pipeline fixes**: (a) expose `/metrics` HTTP route on each service (`Metrics::init()` already installs the global recorder, but nothing serves it today); (b) emit `http_request_duration_seconds` + `http_requests_total` from `HermesOnResponse` (histogram bucket config exists but no code records it); (c) repair `prometheus.yml` (stale `user_profile-service`/`gateway-service` names, missing guild/messaging/realtime targets, wrong ports). |
