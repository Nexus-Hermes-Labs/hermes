...
| 3   | Standardized structured log fields                 | `[ ]` | P0       | Every log line must carry `service`, `request_id`, `user_id` (when authenticated), `trace_id`. Extend `common::observability::tracing` and add a request-context layer that opens a span with these fields per request. |
